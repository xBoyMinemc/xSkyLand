...
// 创建岛屿GUI菜单
function showIslandMenu(player: mc.Player) {
    ...
    mainMenu.show(player as any).then(response => {
...
// 创建岛屿表单
function showCreateIslandForm(player: mc.Player) {
    // 检查玩家是否已经有岛屿
    ...
    createForm.show(player as any).then(response => {
...
// 创建岛屿的具体实现
function createIsland(player: mc.Player, name: string) {
    // 获取新岛屿索引
...
// 传送到自己的岛屿
function teleportToIsland(player: mc.Player) {
    if (xIsLand.GetIsPlayerScore(player.name) <= 0) {
...
// 显示重建确认
function showRemakeConfirmation(player: mc.Player) {
    if (xIsLand.GetIsPlayerScore(player.name) <= 0) {
        player.sendMessage("§c[摆烂空岛] 你还没有自己的岛屿，请先创建一个岛屿");
        showCreateIslandForm(player);
        return;
    ...
    confirmForm.show(player as any).then(response => {
...
// 重建岛屿的具体实现
function remakeIsland(player: mc.Player) {
    // 获取玩家岛屿坐标
...
// 显示岛屿信息
function showIslandInfo(player: mc.Player) {
    ...
    infoForm.show(player as any).then(response => {
...
mc.system.beforeEvents.startup.subscribe((event) => {
    ...
    // 定义一个自定义命令
    const islandCommand: mc.CustomCommand = {
        name: "xisland:x",
        description: "岛屿管理命令",
        cheatsRequired: false,
        permissionLevel: mc.CommandPermissionLevel.Any, // 所有玩家都可以使用
        mandatoryParameters: [
            {
    ...
    // 注册命令及其回调函数
    commandRegistry.registerCommand(islandCommand, (origin, action: string, name?: string) => {
    ...
    overworld.getPlayers().forEach((player) => {
        if(player.location.y < -528) {
            ...
            if(xIsLand.GetIsPlayerScore(player.name) <= 0) {
                ...
                mc.system.runTimeout(() => 
                    player.runCommand(`setblock ${xz[0] * 144 + 74} 38 ${xz[1] * 144 + 74} minecraft:netherrack`), 8);
            }
        }
        ...
        if(player.location.y > 513) {
            ...
            if(xIsLand.GetIsPlayerScore(player.name) <= 0) {
                ...
                mc.system.runTimeout(() => 
                    player.runCommand(`setblock ${xz[0] * 144 + 74} 38 ${xz[1] * 144 + 74} minecraft:end_stone`), 8);
            }
        }
    });
}, 10); // 每半秒检查一次
