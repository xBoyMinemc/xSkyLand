...
const NewMemberIsLand =  (UID : number, MemberName : string) : number =>{ 

    const landUIDName = "##xSky##"+String(UID);
    if (!AssIsLand(landUIDName))return 0;
    
    // r=4，w=2，x=1
    ScoreBase.SetPointsAsync(landUIDName,MemberName,4+2);//设置岛屿中，成员玩家的岛屿活动权
    ScoreBase.SetPointsAsync("##xSkyPlayers##",MemberName,UID);//设置玩家清单中，玩家的岛屿归属，玩家属于哪个岛屿
    return 4+2;
}
