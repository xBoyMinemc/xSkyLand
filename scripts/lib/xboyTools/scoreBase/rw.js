...
const GetScoreObject = (Objective = undefined) => typeof Objective === "undefined" ? world.scoreboard.getObjectives() : typeof Objective === "string" ? world.scoreboard.getObjective(Objective) : Objective;
const GetScorePartic = (args) => args ? (args.dimension ? Array.from(world.scoreboard.getParticipants()).find(Participant => Participant.getEntity() == args) : args[0].getParticipants()) : world.scoreboard.getParticipants();
...
const NewScoreObject = (objectiveId, displayName = objectiveId) => world.scoreboard.addObjective(objectiveId, displayName);
